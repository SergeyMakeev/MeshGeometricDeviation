#pragma once

#include <vector>
#include <algorithm>
#include <limits>
#include <cmath>

// Vector3 utility structure (compatible with Vertex) - using double precision
struct Vector3
{
    double x, y, z;

    Vector3() : x(0), y(0), z(0) {}
    Vector3(double x_, double y_, double z_) : x(x_), y(y_), z(z_) {}

    Vector3 operator+(const Vector3& v) const { return Vector3(x + v.x, y + v.y, z + v.z); }
    Vector3 operator-(const Vector3& v) const { return Vector3(x - v.x, y - v.y, z - v.z); }
    Vector3 operator*(double s) const { return Vector3(x * s, y * s, z * s); }
    Vector3 operator/(double s) const { return Vector3(x / s, y / s, z / s); }

    double dot(const Vector3& v) const { return x * v.x + y * v.y + z * v.z; }
    Vector3 cross(const Vector3& v) const {
        return Vector3(y * v.z - z * v.y, z * v.x - x * v.z, x * v.y - y * v.x);
    }

    double lengthSquared() const { return x * x + y * y + z * z; }
    double length() const { return std::sqrt(lengthSquared()); }

    Vector3 normalized() const {
        double len = length();
        return len > 0 ? (*this) / len : Vector3(0, 0, 0);
    }
};

// Forward declarations
struct Vertex;
struct Triangle;
struct Mesh;

// Bounding box for spatial partitioning
struct BoundingBox
{
    Vector3 min;
    Vector3 max;

    BoundingBox() 
        : min(std::numeric_limits<float>::max(), 
              std::numeric_limits<float>::max(), 
              std::numeric_limits<float>::max()),
          max(-std::numeric_limits<float>::max(), 
              -std::numeric_limits<float>::max(), 
              -std::numeric_limits<float>::max()) {}

    BoundingBox(const Vector3& min_, const Vector3& max_) : min(min_), max(max_) {}

    void expand(const Vector3& point) {
        min.x = std::min(min.x, point.x);
        min.y = std::min(min.y, point.y);
        min.z = std::min(min.z, point.z);
        max.x = std::max(max.x, point.x);
        max.y = std::max(max.y, point.y);
        max.z = std::max(max.z, point.z);
    }

    void expand(const BoundingBox& box) {
        expand(box.min);
        expand(box.max);
    }

    Vector3 center() const {
        return (min + max) * 0.5;
    }

    Vector3 extents() const {
        return (max - min) * 0.5;
    }

    double surfaceArea() const {
        Vector3 d = max - min;
        return 2.0 * (d.x * d.y + d.y * d.z + d.z * d.x);
    }

    // Get the minimum distance from a point to this box (0 if inside)
    double distanceSquared(const Vector3& point) const {
        double dx = std::max(std::max(min.x - point.x, point.x - max.x), 0.0);
        double dy = std::max(std::max(min.y - point.y, point.y - max.y), 0.0);
        double dz = std::max(std::max(min.z - point.z, point.z - max.z), 0.0);
        return dx * dx + dy * dy + dz * dz;
    }
};

// Triangle data for spatial database
struct TriangleData
{
    unsigned int index;      // Original triangle index in mesh
    Vector3 v0, v1, v2;      // Triangle vertices
    Vector3 normal;          // Triangle normal (normalized)
    BoundingBox bounds;      // Triangle bounding box

    TriangleData() : index(0) {}

    TriangleData(unsigned int idx, const Vector3& a, const Vector3& b, const Vector3& c)
        : index(idx), v0(a), v1(b), v2(c)
    {
        bounds.expand(v0);
        bounds.expand(v1);
        bounds.expand(v2);
        
        // Compute triangle normal
        Vector3 edge1 = v1 - v0;
        Vector3 edge2 = v2 - v0;
        normal = edge1.cross(edge2).normalized();
    }

    Vector3 center() const {
        return (v0 + v1 + v2) * (1.0 / 3.0);
    }
};

// KD-Tree node for spatial partitioning
struct KdNode
{
    BoundingBox bounds;
    std::vector<TriangleData> triangles;
    int splitAxis;           // 0=x, 1=y, 2=z, -1=leaf
    double splitPos;
    KdNode* left;
    KdNode* right;

    KdNode() : splitAxis(-1), splitPos(0.0), left(nullptr), right(nullptr) {}

    ~KdNode() {
        delete left;
        delete right;
    }

    bool isLeaf() const { return splitAxis == -1; }
};

// Main Spatial Database class
class SpatialDb
{
private:
    KdNode* root;
    const Mesh* meshPtr;
    std::vector<TriangleData> allTriangles;

    static constexpr int MAX_TRIANGLES_PER_LEAF = 8;
    static constexpr int MAX_DEPTH = 20;

    // Build KD-tree recursively
    KdNode* buildTree(std::vector<TriangleData>& triangles, int depth) {
        if (triangles.empty()) {
            return nullptr;
        }

        KdNode* node = new KdNode();

        // Compute bounding box for all triangles in this node
        for (const auto& tri : triangles) {
            node->bounds.expand(tri.bounds);
        }

        // Leaf node conditions
        if (triangles.size() <= MAX_TRIANGLES_PER_LEAF || depth >= MAX_DEPTH) {
            node->triangles = triangles;
            return node;
        }

        // Choose split axis (longest axis)
        Vector3 extent = node->bounds.extents();
        int axis = 0;
        if (extent.y > extent.x) axis = 1;
        if (extent.z > ((axis == 0) ? extent.x : extent.y)) axis = 2;

        node->splitAxis = axis;

        // Sort triangles by centroid along chosen axis
        std::sort(triangles.begin(), triangles.end(), [axis](const TriangleData& a, const TriangleData& b) {
            Vector3 ca = a.center();
            Vector3 cb = b.center();
            return (axis == 0) ? (ca.x < cb.x) : ((axis == 1) ? (ca.y < cb.y) : (ca.z < cb.z));
        });

        // Split at median
        size_t mid = triangles.size() / 2;
        Vector3 splitCenter = triangles[mid].center();
        node->splitPos = (axis == 0) ? splitCenter.x : ((axis == 1) ? splitCenter.y : splitCenter.z);

        // Partition triangles
        std::vector<TriangleData> leftTris(triangles.begin(), triangles.begin() + mid);
        std::vector<TriangleData> rightTris(triangles.begin() + mid, triangles.end());

        // Recursively build children
        node->left = buildTree(leftTris, depth + 1);
        node->right = buildTree(rightTris, depth + 1);

        return node;
    }

    // Compute closest point on a triangle to a query point
    Vector3 closestPointOnTriangle(const Vector3& p, const TriangleData& tri, double& distSq) const {
        const Vector3& a = tri.v0;
        const Vector3& b = tri.v1;
        const Vector3& c = tri.v2;

        // Check if point projects inside triangle
        Vector3 ab = b - a;
        Vector3 ac = c - a;
        Vector3 ap = p - a;

        double d1 = ab.dot(ap);
        double d2 = ac.dot(ap);
        if (d1 <= 0.0 && d2 <= 0.0) {
            distSq = ap.lengthSquared();
            return a; // Closest to vertex a
        }

        // Check if closest to vertex b
        Vector3 bp = p - b;
        double d3 = ab.dot(bp);
        double d4 = ac.dot(bp);
        if (d3 >= 0.0 && d4 <= d3) {
            distSq = bp.lengthSquared();
            return b;
        }

        // Check if closest to edge ab
        double vc = d1 * d4 - d3 * d2;
        if (vc <= 0.0 && d1 >= 0.0 && d3 <= 0.0) {
            double v = d1 / (d1 - d3);
            Vector3 closest = a + ab * v;
            distSq = (p - closest).lengthSquared();
            return closest;
        }

        // Check if closest to vertex c
        Vector3 cp = p - c;
        double d5 = ab.dot(cp);
        double d6 = ac.dot(cp);
        if (d6 >= 0.0 && d5 <= d6) {
            distSq = cp.lengthSquared();
            return c;
        }

        // Check if closest to edge ac
        double vb = d5 * d2 - d1 * d6;
        if (vb <= 0.0 && d2 >= 0.0 && d6 <= 0.0) {
            double w = d2 / (d2 - d6);
            Vector3 closest = a + ac * w;
            distSq = (p - closest).lengthSquared();
            return closest;
        }

        // Check if closest to edge bc
        double va = d3 * d6 - d5 * d4;
        if (va <= 0.0 && (d4 - d3) >= 0.0 && (d5 - d6) >= 0.0) {
            double w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
            Vector3 closest = b + (c - b) * w;
            distSq = (p - closest).lengthSquared();
            return closest;
        }

        // Point projects inside triangle
        double denom = 1.0 / (va + vb + vc);
        double v = vb * denom;
        double w = vc * denom;
        Vector3 closest = a + ab * v + ac * w;
        distSq = (p - closest).lengthSquared();
        return closest;
    }

    // Recursively search KD-tree for closest point
    void searchNode(const KdNode* node, const Vector3& query, Vector3& closestPoint, double& minDistSq) const {
        if (!node) return;

        // Early exit if this node's bounding box is farther than current best
        double boxDistSq = node->bounds.distanceSquared(query);
        if (boxDistSq >= minDistSq) {
            return;
        }

        if (node->isLeaf()) {
            // Test all triangles in this leaf
            for (const auto& tri : node->triangles) {
                double distSq;
                Vector3 candidate = closestPointOnTriangle(query, tri, distSq);
                if (distSq < minDistSq) {
                    minDistSq = distSq;
                    closestPoint = candidate;
                }
            }
        } else {
            // Determine which child to search first based on query position
            double queryPos = (node->splitAxis == 0) ? query.x : 
                           ((node->splitAxis == 1) ? query.y : query.z);
            
            KdNode* nearChild = (queryPos < node->splitPos) ? node->left : node->right;
            KdNode* farChild = (queryPos < node->splitPos) ? node->right : node->left;

            // Search near child first
            searchNode(nearChild, query, closestPoint, minDistSq);
            
            // Only search far child if it could contain a closer point
            searchNode(farChild, query, closestPoint, minDistSq);
        }
    }

    // Recursively search KD-tree for closest point with normal filtering
    void searchNodeWithNormal(const KdNode* node, const Vector3& query, const Vector3& desiredNormal, 
                             double minDotProduct, Vector3& closestPoint, double& minDistSq) const {
        if (!node) return;

        // Early exit if this node's bounding box is farther than current best
        double boxDistSq = node->bounds.distanceSquared(query);
        if (boxDistSq >= minDistSq) {
            return;
        }

        if (node->isLeaf()) {
            // Test all triangles in this leaf
            for (const auto& tri : node->triangles) {
                // Check if triangle normal is pointing in approximately the same direction
                double dotProduct = tri.normal.dot(desiredNormal);
                if (dotProduct >= minDotProduct) {
                    double distSq;
                    Vector3 candidate = closestPointOnTriangle(query, tri, distSq);
                    if (distSq < minDistSq) {
                        minDistSq = distSq;
                        closestPoint = candidate;
                    }
                }
            }
        } else {
            // Determine which child to search first based on query position
            double queryPos = (node->splitAxis == 0) ? query.x : 
                           ((node->splitAxis == 1) ? query.y : query.z);
            
            KdNode* nearChild = (queryPos < node->splitPos) ? node->left : node->right;
            KdNode* farChild = (queryPos < node->splitPos) ? node->right : node->left;

            // Search near child first
            searchNodeWithNormal(nearChild, query, desiredNormal, minDotProduct, closestPoint, minDistSq);
            
            // Only search far child if it could contain a closer point
            searchNodeWithNormal(farChild, query, desiredNormal, minDotProduct, closestPoint, minDistSq);
        }
    }

public:
    SpatialDb() : root(nullptr), meshPtr(nullptr) {}

    SpatialDb(const Mesh& mesh) : root(nullptr), meshPtr(&mesh) {
        build(mesh);
    }

    ~SpatialDb() {
        delete root;
    }

    // Build the spatial database from a mesh
    void build(const Mesh& mesh);

    // Result structure for normal-filtered queries
    struct ClosestPointResult {
        Vector3 point;              // The closest point found
        bool matchedNormalConstraint; // True if the result matched the normal constraint, false if fallback was used
        
        ClosestPointResult() : point(0, 0, 0), matchedNormalConstraint(false) {}
        ClosestPointResult(const Vector3& p, bool matched) : point(p), matchedNormalConstraint(matched) {}
    };

    // Query the closest point on the mesh surface to an arbitrary point
    Vector3 getClosestPointOnMeshSurface(const Vector3& queryPoint) const {
        if (!root) {
            return Vector3(0, 0, 0); // Empty database
        }

        Vector3 closestPoint;
        double minDistSq = std::numeric_limits<double>::max();
        
        searchNode(root, queryPoint, closestPoint, minDistSq);
        
        return closestPoint;
    }

    // Query the closest point on the mesh surface, filtering by triangle normal direction
    // desiredNormal: The desired normal direction (will be normalized internally)
    // maxAngleDegrees: Maximum angle deviation allowed (in degrees, e.g., 45.0 for ±45 degrees)
    // Returns: ClosestPointResult containing the point and whether it matched the normal constraint
    //          If no triangles match the normal constraint, falls back to searching all triangles
    ClosestPointResult getClosestPointOnMeshSurface(const Vector3& queryPoint, const Vector3& desiredNormal, 
                                                     double maxAngleDegrees = 45.0) const {
        if (!root) {
            return ClosestPointResult(Vector3(0, 0, 0), false); // Empty database
        }

        // Normalize the desired normal
        Vector3 normalizedDesiredNormal = desiredNormal.normalized();
        
        // Convert angle to dot product threshold
        // cos(angle) gives us the dot product for aligned normals
        double maxAngleRadians = maxAngleDegrees * 3.14159265358979323846 / 180.0;
        double minDotProduct = std::cos(maxAngleRadians);

        Vector3 closestPoint;
        double minDistSq = std::numeric_limits<double>::max();
        
        searchNodeWithNormal(root, queryPoint, normalizedDesiredNormal, minDotProduct, closestPoint, minDistSq);
        
        // Fallback: If no triangles matched the normal constraint, search without filtering
        if (minDistSq == std::numeric_limits<double>::max()) {
            searchNode(root, queryPoint, closestPoint, minDistSq);
            return ClosestPointResult(closestPoint, false); // Used fallback
        }
        
        return ClosestPointResult(closestPoint, true); // Matched normal constraint
    }

    // Get the distance from a point to the nearest point on the mesh surface
    double getDistanceToSurface(const Vector3& queryPoint) const {
        Vector3 closest = getClosestPointOnMeshSurface(queryPoint);
        return (queryPoint - closest).length();
    }

    // Get statistics about the spatial database
    struct Stats {
        int totalNodes;
        int leafNodes;
        int totalTriangles;
        int maxDepth;
        int minTrisPerLeaf;
        int maxTrisPerLeaf;
        float avgTrisPerLeaf;
    };

    Stats getStats() const {
        Stats stats = {0, 0, 0, 0, 
                      std::numeric_limits<int>::max(), 
                      0, 0.0f};
        
        if (root) {
            collectStats(root, stats, 0);
            if (stats.leafNodes > 0) {
                stats.avgTrisPerLeaf = static_cast<float>(stats.totalTriangles) / stats.leafNodes;
            }
        }
        
        return stats;
    }

private:
    void collectStats(const KdNode* node, Stats& stats, int depth) const {
        if (!node) return;

        stats.totalNodes++;
        stats.maxDepth = std::max(stats.maxDepth, depth);

        if (node->isLeaf()) {
            stats.leafNodes++;
            int triCount = static_cast<int>(node->triangles.size());
            stats.totalTriangles += triCount;
            stats.minTrisPerLeaf = std::min(stats.minTrisPerLeaf, triCount);
            stats.maxTrisPerLeaf = std::max(stats.maxTrisPerLeaf, triCount);
        } else {
            collectStats(node->left, stats, depth + 1);
            collectStats(node->right, stats, depth + 1);
        }
    }
};

