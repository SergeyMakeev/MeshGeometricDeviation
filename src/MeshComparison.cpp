    ...
    if (hasNormals)
    {
        ...
        for (unsigned int faceIdx = 0; faceIdx < obj->face_count; faceIdx++)
        {
            ...
            for (unsigned int i = 0; i < vertCount; i++)
            {
                ...
                {
                    Vector3 normal(obj->normals[3 * (normIdx - 1) + 0], obj->normals[3 * (normIdx - 1) + 1],
                            ...
                            bool useNormalFiltering, unsigned int seed, bool outerShellOnly, int spherePoints)
{
    ...
    log(LogLevel::Info, "\nMeasuring distances");
    if (hasVertexNormals)
        log(LogLevel::Info, " + normal variance");
    if (hasUVs)
        log(LogLevel::Info, " + UV variance");
    log(LogLevel::Info, "...\n");
    ...
    for (const SurfaceSample& sample : samples)
    {
        ...
        if (useNormalFiltering)
        {
            result = spatialDb.getClosestPointDetailedWithNormal(sample.position, sample.normal, maxAngleDegrees);
        ...
        if (hasVertexNormals)
        {
            ...
            Vector3 sampleVertexNormal = meshA.vertexNormals[sampleTri->i0] * sample.baryU +
            ...
            Vector3 closestVertexNormal = meshB.vertexNormals[closestTri->i0] * result.baryU +
        ...
        if (hasUVs)
        {
            ...
            if (sampleTri->t0 > 0 && sampleTri->t1 > 0 && sampleTri->t2 > 0 && closestTri->t0 > 0 && closestTri->t1 > 0 &&
                closestTri->t2 > 0)
            {
...
void printDevianceStats(const DevianceStats& stats, bool showNormalStats)
{
    ...
    if (showNormalStats && stats.fallbackCount >= 0)
    {
        ...
        log(LogLevel::Debug, "  Fallback: %d samples (%.2f%%)\n", stats.fallbackCount, 100.0f * stats.fallbackCount / stats.totalSamples);
    }
...
void exportDebugVisualization(const std::string& filename, const Mesh& meshA, const Mesh& meshB, const BidirectionalDevianceStats& biStats)
{
    ...
    log(LogLevel::Debug, "  Sample position: (%g, %g, %g)\n", maxDistanceCase.samplePosition.x, maxDistanceCase.samplePosition.y,
        maxDistanceCase.samplePosition.z);
    log(LogLevel::Debug, "  Closest position: (%g, %g, %g)\n", maxDistanceCase.closestPosition.x, maxDistanceCase.closestPosition.y,
        maxDistanceCase.closestPosition.z);

    if (hasVertexNormals && maxNormalAngleCase.value > 0.0)
    {
        log(LogLevel::Info, "\nMax Normal Angle: %g degrees\n", maxNormalAngleCase.value);
        log(LogLevel::Debug, "  Sample position: (%g, %g, %g)\n", maxNormalAngleCase.samplePosition.x, maxNormalAngleCase.samplePosition.y,
            maxNormalAngleCase.samplePosition.z);
        log(LogLevel::Debug, "  Closest position: (%g, %g, %g)\n", maxNormalAngleCase.closestPosition.x,
            maxNormalAngleCase.closestPosition.y, maxNormalAngleCase.closestPosition.z);
    }

    if (hasUVs && maxUVDistanceCase.value > 0.0)
    {
        log(LogLevel::Info, "\nMax UV Distance: %g\n", maxUVDistanceCase.value);
        log(LogLevel::Debug, "  Sample position: (%g, %g, %g)\n", maxUVDistanceCase.samplePosition.x, maxUVDistanceCase.samplePosition.y,
            maxUVDistanceCase.samplePosition.z);
        log(LogLevel::Debug, "  Closest position: (%g, %g, %g)\n", maxUVDistanceCase.closestPosition.x, maxUVDistanceCase.closestPosition.y,
            maxUVDistanceCase.closestPosition.z);
    }
